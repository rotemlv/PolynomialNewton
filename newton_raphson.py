...
def newtons_method(x0, f, f_prime, tolerance, epsilon, max_iterations, do_verbose=False):
    ...
    for _ in range(max_iterations):
        yprime = f_prime(x0)
        if abs(yprime) < epsilon:  # Give up if the denominator is too small
            xxprint("Failed in derivative value (too close to zero)!")
            return None
        try:
            x1 = x0 - f.division_eval(f_prime, x0)  # Do Newton's computation
        except RuntimeWarning as w:
            if "divide by zero" in str(w):
                xxprint("Failed in derivative value calculation (got 0 in denominator)")
                return None
    ...
    xxprint(f"Failed in max-iterations for {original_guess=}, reached: {x1 if max_iterations>0 else x0}")
